...
# Images
SLIDER = pygame.Surface((1000, 1000), pygame.SRCALPHA)
COLORPOS = (85, 85)
VALUEPOS = (515, 830)


def get_color(colorpos, valuepos, picker):
...
def draw_window(win, width, height, picker_changed, slider_changed, erasing, square_cursor, eraser, clear, picker, currentcolor):
    ...
    if picker_changed:
        draw_slider(15 + BOXWIDTH*2 + 15 + 15 + 80 + picker.get_width() + 10, height - BOTTOMBARHEIGHT + 5, 20, picker.get_height(), currentcolor)
        picker_changed = False
    if slider_changed:
        picker = update_picker(picker, currentcolor)
    ...
    return picker, square_box, circle_box, slider_box, eraser_box, clear_box, (15 + BOXWIDTH*2 + 15 + 15 + 80 + picker.get_width() + 10, height - BOTTOMBARHEIGHT + 5, 20, picker.get_height()), picker_changed, slider_changed
...
def draw_slider(x, y, width, height, currentcolor):
    global SLIDER
...
def update_picker(picker, currentcolor):
    w, h = picker.get_size()
    for x in range(w):
        for y in range(h):
...
def main(win, width, height, eraser, clear, picker):
    global BRUSHSIZE, COLORPOS, VALUEPOS, CURRENTCOLOR
    win.fill(WHITE)
    CURRENTCOLOR = get_color(COLORPOS, VALUEPOS, picker)
    ...
    run = True
    while run:
        picker, square_box, circle_box, slider_box, eraser_box, clear_box, value_box, picker_changed, slider_changed = draw_window(win, width, height, picker_changed, slider_changed, erasing, square_cursor, eraser, clear, picker, CURRENTCOLOR)
